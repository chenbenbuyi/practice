 ...
 * @Description 将 refresh()、getBean()、registerBeanDefinition() 等方法提取到抽象类，以实现代码复用
 */

public abstract class AbstractBeanFactory extends DefaultSingletonBeanRegistry implements BeanFactory, BeanDefinitionRegistry {
    protected Map<String, BeanDefinition> beanDefinitionMap = new
            ConcurrentHashMap<>(256);
    protected List<String> beanDefinitionNames = new ArrayList<>();

    /**
     * Sprign中Bean的创建过程：解析Bean定义生成BeanDefinition，根据定义实例化，然后进行属性注入
     * 所以，属性注入之前，其实这个 Bean 的实例已经创建好了，只不过还有很多属性没有值，可以说是一个早期的毛坯实例
     * 于是，在Spring中处理循环依赖时，引入了earlySingletonObjects这样一个三级缓存，用于存放早起的毛坯实例。创建 Bean 实例的时候，不用等到所有步骤完成，而是可以在属性还没有注入之前，就把早期的毛胚实例先保存起来，供属性注入时使用
     * <p>
     * 扩展：这也就是为什么Spring中，构造器注入无法解决循环依赖的问题，只有属性方式注入才能解决循环依赖的问题。
     * 实际情况中，要解决构造器的循环依赖，要么修改自己的业务代码，要么通过添加@Lazy注解进行延迟加载（参考:https://zhuanlan.zhihu.com/p/562691467?utm_id=0）
     */
    ...
    /**
     * 该包装了整个 Bean 的创建过程：在创建 Bean 实例之后，要进行初始化工作，refresh() 方法内预留了 postProcessBeforeInitialization、init-method 与 postProcessAfterInitialization 的位置，根据方法名称可以看出是在初始化前、中、后分别对 Bean 进行处理
     * <p>
     * 因为Autowired注解是作用在一个实例变量上的，为了让该注解生效实现对象的注入，必须创建好这个对象实例，所以可以 createBean 之后完成该注解生效的相关逻辑
     * 而为了进一步抽象，定义了一个Bean的处理器接口{@link BeanPostProcessor} 用来处理注解
     */
    public void refresh() {
